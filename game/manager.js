const {
  spawn: spawnNewGuildData,
  liveify,
} = require(`./basics/guild/guild`)
...
module.exports = {
  ...
  addGuild(newGuild) {
    ...
    if (existingGuildInGame) {
      ...
      return {
        ok: false,
        message: story.ship.get.fail.existing(
          existingGuildInGame,
        ),
  ...
  async spawn({ discordGuild, channelId }) {
    const existingGuildInDb = await this.getGuild(
      discordGuild.id,
    )
  ...
  verifyActiveGuilds(discordGuilds) {
    setTimeout(() => {
      this.guilds.forEach((g) => {
        if (
          !discordGuilds.find(
            (discordGuild) => discordGuild.id === g.id,
          )
        ) {
